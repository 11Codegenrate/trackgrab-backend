...
## Version 1.3.6: what this build fixes
...
This release fixes downloader process and queue behavior. It can download full, unprotected audio that SoundCloud makes available from the VPS location. It cannot make DRM-protected, region-blocked, private, deleted, or preview-only tracks available. The prepared archive is not evidence of a live deployment.
...
- Transient errors may be retried within the job timeout. DRM, geographic restrictions, private tracks, and preview-only audio are not retried. Optional artwork/tag failures can trigger a fresh attempt without metadata rather than serving an unverified leftover file.
...
This earlier deployment note is retained for troubleshooting and was not reverified for version 1.3.6. It is separate from the DRM/geo/interrupted-download errors supplied for this fix.
...
Diagnostics must report `"version":"1.3.6"` and `ytdlp.ok`, `ffmpeg.ok`, and `ffprobe.ok` all `true`. `--update-env` applies the deployment environment; it does not update source files or install dependencies. Check `pm2 logs trackgrab --lines 80 --nostream` after the reload.
...
yt-dlp --ignore-config --no-playlist --format 'bestaudio[format_id!*=preview]/best[format_id!*=preview]' -x --audio-format mp3 --audio-quality 320K --print 'after_move:COMPLETE:%(filepath)s' -o '/tmp/trackgrab-check.%(ext)s' -- 'https://soundcloud.com/ARTIST/PUBLIC-TRACK'
```
