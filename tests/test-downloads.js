...
const classifications = [
  ...
  ["a discarded DRM warning cannot hide a later network error", "WARNING: One format is DRM protected\nERROR: HTTP Error 503: Service Unavailable", {}, "upstream_unavailable", true],
  ["preview in a track URL does not prevent network retry", "ERROR: unable to download https://soundcloud.com/artist/preview-private: HTTP Error 403: Forbidden", {}, "upstream_forbidden", true],
  ["preview in a title does not prevent conversion diagnostics", "ERROR: Postprocessing: ffmpeg failed for Private Preview", {}, "conversion_failed", false],
  ["explicit private access failure", "ERROR: This track is private", {}, "track_private", false],
];
...
test("HTTP download regressions use the real queue and temporary files", { timeout: 30000 }, async (t) => {
  ...
  try {
    ...
    await t.test("track info reports whether only preview streams are exposed", async () => {
      for (const [formats, expected] of [
        [[{ format_id: "hls_mp3_128k_preview", url: "https://media.example/preview" }], true],
        [[{ format_id: "hls_mp3_128k_preview", url: "https://media.example/preview" }, { format_id: "hls_aac_160k", url: "https://media.example/full" }], false],
        [undefined, null],
      ]) {
        plans.push({ metadata: { title: "Test track", formats } });
        const result = await request(`/info?${new URLSearchParams({ url: "https://soundcloud.com/artist/test-track" })}`, { "X-API-Key": sharedSecret }).pending;
        assert.equal(result.status, 200);
        assert.equal(JSON.parse(result.body).preview_only, expected);
      }
      reset();
    });

    await t.test("available preview streams download and are clearly labeled", async () => {
      plans.push({ sourceFormat: "hls_mp3_128k_preview" });
      const result = await request(signedPath(), { Origin: "https://example.com" }).pending;
      assert.equal(result.status, 200);
      assert.deepEqual(result.body, audio);
      assert.equal(result.headers["x-trackgrab-preview"], "1");
      assert.match(result.headers["content-disposition"], /Test track \(preview\)\.mp3/);
      assert.match(result.headers["access-control-expose-headers"], /X-TrackGrab-Preview/i);
      const args = calls[0].args;
      assert.equal(args[args.indexOf("--format") + 1], "bestaudio[format_id!*=preview]/best[format_id!*=preview]/bestaudio/best");
      assert.ok(args.includes("--check-formats"));
      assert.ok(args.includes("soundcloud:formats=*"));
      assert.ok(args.includes("after_move:TRACKGRAB_SOURCE:%(format_id)s"));
      reset();
    });
    ...
    await t.test("all supported output formats retain their filenames and MIME types", async () => {
      ...
      for (const [format, mime] of Object.entries(formats)) {
        ...
        assert.equal(result.headers["x-trackgrab-preview"], "0");
        assert.deepEqual(result.body, audio);
      }
